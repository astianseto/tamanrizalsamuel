...
<main class="max-w-4xl mx-auto bg-white rounded-2xl shadow-md mt-10 p-8">
    ...
    {{-- Gambar --}}
    @if(!empty($aduan->file))
        <a href="{{ asset('storage/' . $aduan->file) }}" target="_blank">
    ...
    {{-- Jawaban Aduan --}}
    @php
        $detail = $aduan->detailAduan->first(); // ambil jawaban pertama
    @endphp

    <div class="mt-8 border-t pt-5">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Jawaban Aduan</h3>
        @if($detail && !empty($detail->jawaban))
    ...
    <div class="mt-10 border-t pt-5">
        ...
        @php
            $status = strtolower($detail->status ?? 'ditinjau');
        @endphp
